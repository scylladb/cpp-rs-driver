...
impl CassPrepared {
    pub(crate) fn new_from_prepared_statement(mut statement: PreparedStatement) -> Self {
        ...
        // For older versions in other driver we usually disabling this option in order to
        // err on the side of correctness rather than performance.
        // Here it is more difficult, because the driver does it's own checking, and transforms
        // metadata in expensive way. Disabling caching would require more changes, so lets
